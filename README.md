...
一款温暖简约的日程管理应用，支持 **Windows 桌面版** 与 **Android 手机版**，手机与电脑通过云端**随时随地自动同步**。

- 桌面版：Electron
- 手机版：Capacitor 8（同一套 HTML 打包为 APK）
- 同步：零依赖 Node 同步服务器（已部署官方免费服务器，开箱即用）
...
应用内置**官方默认同步服务器**：`https://scheduler-new.onrender.com` —— 两端打开即自动同步，**无需任何配置**。
